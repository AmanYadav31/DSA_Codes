...
Time Complexity: O((N*M)*(N + M)) + O(N*M), where N = no. of rows in the matrix and
...
Reason: Firstly, we are traversing the matrix to find the cells with the value 0.
 It takes O(N*M). Now, whenever we find any such cell we mark that row and column with -1.
  This process takes O(N+M). So, combining this the whole process, finding and marking, takes 
  O((N*M)*(N + M)).
...
/*Better approach=>
Time Complexity: O(2*(N*M)), where N = no. of rows in the matrix and M = no. of columns in the
matrix.
...
Reason: In this approach, we are also traversing the entire matrix 2 times and each traversal is
