...
Time Complexity: O( 4*N*M * log( N*M) ) { N*M are the total cells, for each of which we also check 4 
adjacent nodes for the minimum effort and additional log(N*M) for insertion-deletion operations in a 
priority queue } 
...
Where, N = No. of rows of the binary maze and M = No. of columns of the binary maze.*/
    int MinimumEffort(vector<vector<int>> &heights)
    {
        ...
        while (!pq.empty())
        {
            ...
            if (row == n - 1 && col == m - 1)//Jab bhi yahaan pe pahunchenge toh min-heap ki wajah se
                               //least effort waala hi encounter hoga. Dry Run to understand properly.
                return diff;
